...
import { infuraUriRinkby, ethRinkby, GAMEADDRESSRinkby } from "./config/config.js";
...
export default class Web3Class extends cc.Component {
  ...
  initContracts() {
    this.GameContract = new this.web3.eth.Contract(GameABI, GAMEADDRESSRinkby);
    ...
    if (this.GameContract) {
      this.GameContract.methods
        ...
        .send({
          ...
          value: my.web3.utils.toWei("0.001", "ether"),
        })
  ...
  async ExpandCap() {
    ...
    if (this.GameContract) {
      this.GameContract.methods
        ...
        .send({
          from: my.currentAccount,
          gas: 3000000,
          value: my.web3.utils.toWei("0.0005", "ether"),
        })
        .on("receipt", function (receipt) {
          console.log(receipt);
          // 复活
          if(cc.find("Canvas/UI").getComponent("UI")){
            cc.find("Canvas/UI").getComponent("UI").continueGameForWeb3()
          }
    ...
    async getChainLinkRandomNum(){
      let result = Math.random()*1000000000000
      if (this.GameContract) {
        let block = await this.GameContract.methods.totalBlock().call();
        result = await this.GameContract.methods.getUserRandomNum(block).call();
      }
      return result;
    }
  ...
  async setWeb3Provider() {
    if (WinEthereum) {
      ...
      this.web3Provider = new Web3.providers.HttpProvider(infuraUriRinkby);
    }
    return 1;
  }
  ...
  async switch() {
    ...
    await this.switchChain(ethRinkby);
  }
