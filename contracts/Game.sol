...
interface IVRFv2ConsumerCaller {
    function requestRandomWordsToCaller() external returns(uint256);
    function requestRandomWords() external;
    function getSRequestId()external returns(uint256);
    function getSRandomWords(uint index)external returns(uint256);
}
...
contract Game is GovManager {
    ...
    IVRFv2ConsumerCaller IVRFv2Consumer;
    ...
    constructor (address payable _dao,address IVRFv2ConsumerCallerAddr) public{
        totalPlayer = 0;
        totalBlock = 0;
        DAO = _dao;
        IVRFv2Consumer = IVRFv2ConsumerCaller(IVRFv2ConsumerCallerAddr);
    }

    event payEvent(uint time,address sender,uint eventType,uint value);

    function RandomWordsToCaller() internal returns(uint256){
        return IVRFv2Consumer.requestRandomWordsToCaller();
    }

    function getRandomNum() public returns(uint256) {
        return IVRFv2Consumer.getSRandomWords(0);
    }

    function StartGame() public payable {
        require(msg.value >= 5*10**14,"matic value error");
        ...
        GameBlocks[totalBlock].Players[msg.sender].requestId = RandomWordsToCaller();
        GameBlocks[totalBlock].Players[msg.sender].random = getRandomNum();
        players[totalPlayer] = msg.sender;
        totalPlayer = totalPlayer + 1;
    }

    function getUserRandomNum(uint blockNUm)external returns(uint256) {
        return GameBlocks[blockNUm].Players[msg.sender].random;
    }


    function distribution(address payable winner) internal {
        winner.transfer(5*10**17);
        DAO.transfer(5*10**17);
    }

    function Expand() public payable {
        require(msg.value >= 5*10**13,"matic value error");
